        ...
        with open('violation.csv', 'r') as csvr:
            ...
            for row in reader:
                if (filename2==row['Vid']):
                    ...
                    display3.config(text=(row['make']))
            csvr.close()
        fname = filename2[:-4]
        if int(fname.strip("Case ")) <= c:
        ...
        with open('violation.csv', 'a') as csvw:
            with open('Violated/'+filename2, 'rb') as i:
                ...
                try:
                    ...
                    make = js['results'][0]['vehicle']['make'][0]['name']
                    body_type= js['results'][0]['vehicle']['body_type'][0]['name']
                    color = js['results'][0]['vehicle']['color'][0]['name']
                    model = js['results'][0]['vehicle']['make_model'][0]['name']
                    row = [filename2,LP,lx1,ly1,lx2,ly2,vx,vy,vh,vw,make,body_type,color,model]
                    s.add(filename2)
...
def stream(label):
    ...
    while True:
        _,iframe = cap.read()
        if(snap%2==0):
            cv2.imwrite("Violated/Case {}.png".format(c),iframe)
            s=s+1
            snap=1
        if(mode==1):
